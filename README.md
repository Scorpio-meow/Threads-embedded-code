...
- 偵測失效貼文、重新導向與 fallback summary，過濾作者個人簡介與粉絲數等後設資料，避免把錯誤內容或 Bio 當成貼文內文
...
- 排除後設資料干擾：智能識別並過濾作者簡介 (Bio)、粉絲數、串文數（例如 "X 位粉絲 • Y 則串文"）與 Threads 平台引導語，避免在貼文內文為空時誤擷取非貼文的資訊
- 自動抽取：
   - 貼文內文
   - 作者與作者連結
   - 發文時間與時間標題
