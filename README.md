...
- **Popup 與 Dashboard 雙介面**：
  ...
  - **全頁儀表板 (`dashboard.html`)**：提供完整的貼文管理、全文檢索、多重排序、標籤篩選、批次操作與維護工具。
  - **作者與標籤統計雲**：儀表板新增「作者貼文計數統計（Authors Cloud）」與「熱門標籤雲（Tags Cloud）」，自動統計儲存資料中排名前 15 名的作者與標籤，並支援點擊一鍵快速篩選相關貼文。
- **無行內樣式與嚴格 CSP 相容**：全面移除網頁中的行內樣式（Inline Styles），改由獨立的外部 CSS 檔案（`dashboard.css` 與 `popup.css`）統一控制。完美相容於瀏覽器嚴格的 Content Security Policy (CSP) 規範，避免任何潛在的安全限制與代碼品質警告。
...
- **作者與標籤統計雲渲染**：讀取本機 `savedArticles` 中所有文章，將作者（`author`）與標籤（`tags`）分別進行出現次數統計。依據貼文數量由多到少遞減排序，擷取前 15 名的熱門作者與標籤在儀表板頂端渲染為微型徽章（Badge），並標註該項目的貼文計數（例如 `@username (5)` 或 `#JavaScript (3)`），點擊徽章可直接進行動態過濾。
